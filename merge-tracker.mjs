...
import { normalizeStatusMeta } from './lib/statuses.mjs';
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const col4LooksLikeStatus = normalizeStatusMeta(col4, CAREER_OPS).recognized;
    const col5LooksLikeStatus = normalizeStatusMeta(col5, CAREER_OPS).recognized;
...
async function main() {
  // --- Pre-lock checks (no applications.md lock needed yet) ---
  ...
  // --- Acquire lock on applications.md before read+write ---
  let release;
  try {
  ...
  try {
    // Read applications.md (inside the lock)
    ...
    // Write back (still inside the lock)
    if (!DRY_RUN) {
  ...
  // Optional verify (outside the lock — read-only)
  if (VERIFY && !DRY_RUN) {
